...
import random
import json
import socket  # Add this import with other imports at the top
...
ACTIVE_VPS_COUNT = 5  # डिफॉल्ट रूप से 6 VPS इस्तेमाल होंगे
...
START_IMAGES = [
    {
        'url': 'https://files.catbox.moe/pi6bzz.jpg',
        'caption': (
...
async def add_vps_info(update: Update, context: CallbackContext):
    try:
        ...
        # Test SSH connection before adding to VPS list
        try:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(ip, username=username, password=password, timeout=10)
            
            # Test if the connection works by executing a simple command
            ...
            await update.message.reply_text(
                ...
                f"IP: `{ip}`\n"
                f"Error: {str(e)}\n\n"
                f"*Please check if SSH service is running on port 22.*",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
            ...
            await update.message.reply_text(
                ...
                f"IP: `{ip}`\n\n"
                f"*The VPS is not responding on port 22.*",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
        ...
        # If connection test passes, add VPS to list
...
# Add this handler with your other handlers
    ...
    # Manual key expiration check (run once at startup)
    check_expired_keys(None)
    logging.info("Initial key expiration check completed")
